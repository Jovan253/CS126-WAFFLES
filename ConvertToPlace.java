...
import uk.ac.warwick.cs126.structures.HashMap; // import hashmap
...
public class ConvertToPlace implements IConvertToPlace {
    ...
    HashMap<Double, Place> hashMap;

    public ConvertToPlace() {
        // Initialise things here
        hashMap = new HashMap<>();
        Place[] place = getPlacesArray();            
        for (int i=1; i<place.length;i++){
            hashMap.add((Math.pow(2,place[i].getLatitude())*Math.pow(3, place[i].getLongitude())), place[i]);
        } 
        
    }

    public Place convert(float latitude, float longitude) {
        Place place = hashMap.get((Math.pow(2,latitude)*Math.pow(3, longitude)));        
        if (place ==null){
